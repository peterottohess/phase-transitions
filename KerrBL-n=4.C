/*
    ...
    Modified Version by Thomas Schönenbach 2013-2014
...
double KerrBL::getRmb() const {
  ...
  // needs to be fixed to include the real rmb
    throwError( "KerrBL.C : should never come here to find marginally bound orbit" );
    return (inneredge(spin_));
  }
  else
...
// assumed to be at 3./2. + drhor
double KerrBL::inneredge(const double aa) const 
{
     return(3./2. +drhor);
//   if (aa == 0.0) 
//    {
...
int KerrBL::diff(const double* coordGen, const double* cst, double* res) const{
  ...
  // January 2014: actually it changes some thing... not sure what at the moment
  double rsink=1.+sqrt(1.-a2)+drhor;
 // TS this is not the bug I'm looking for...
 // connected to negative Fluxes
  if (pseudoB_ != 0)
   {
     rsink = inneredge(spin_) ;
//      rsink = 2.0;
   }
...
#   endif
//  check if this code actually is executed 
//      cout << "Program came here";
    return 1;
  }
...
# endif
  ...
  vel[1] = vel[2] = 0.;
    // vel[3] = 1./((dir*pow(coord[1], 1.5) + spin_)*sinth); //TS Modified to incorporate
							     ...
							     // corresponds to beeing at the equator?! see eq.(5) in MNTRAS-II-2019
  vel[3] = 1./((dir * pow( (2.*pow(coord[1],6))/(2. * coord[1] * coord[1]*coord[1] -(2./3.) * pseudoB_),0.5) + spin_) * sinth) ;
//   cout << "Winkelgeschwindigkeit vorher" << vel[3] << "\n"; 
  vel[0] = SysPrimeToTdot(coor, vel+1);
   if ( vel[0] == 0) 
   {
//      cout << "angular velocity: " << vel[3] << " and sintheta: " << sinth << "\n";
     cout <<"coor=["<<coor[0]<<", "<<coor[1]<<", "<<coor[2]<<", "<<coor[3]
	     <<"], dir="<<dir<<endl;
     cout << "pseudoB = " << pseudoB_ << "\n" ;
   }
  vel[3] *= vel[0];
# if GYOTO_DEBUG_ENABLED
...
//	 gtt=-(1.-(2.*rr +B/(6.*pow(rr,2)))/Sigma), 
//	 gtp=-(2.*rr - B/(6.*pow(rr,2)))*aa*sintheta2/Sigma, 
//	 gpp = sintheta2*(r2+a2+(2.*rr - B/(6.*pow(rr,2)))*a2*sintheta2/Sigma), 
//TS new
...
/*  EE=(1-(2*rr - B/(6.*rr*rr))/Sigma)*tdot+fact*phidot; 
